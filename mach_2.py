...
class Inevitable:
    ...
    def Start( self ):
        ...
        if self.game.register( username = 'Mach_2', password = "LeilaniFlores" ):
            while True:
                if self.Refresh():
                    self.FetchInfo()
                    self.GameLoop()
    ...
    def GameLoop( self ):
        print( "Energy Source: " + str( self.me.energy_source - self.me.tax_amount ) )
        print( "Energy Source: " + str( self.me.gold_source - self.me.tax_amount ) )
        print( "" )
        self.Defend()
        if self.tech == 1:
            self.Expand()
